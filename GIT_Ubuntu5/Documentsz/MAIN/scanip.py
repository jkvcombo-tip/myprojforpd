...
def convert_syslog_to_csv(syslog_file, csv_output_file):
    converter.convert_to_csv(syslog_file, csv_output_file)
    print(f"Syslog converted to CSV: {csv_output_file}")
...
if __name__ == "__main__":
    ...
    # Save reachable devices to Ansible inventory
    save_to_inventory(reachable_devices, inventory_file)

    # Run Ansible playbook
    run_ansible_playbook(inventory_file, playbook_file)

    # Convert syslog from the local device to CSV
    local_syslog_file = './SYSLOG/END DEVICE/syslog.txt'
    local_csv_output_file = './SYSLOG/END DEVICE/syslog.csv'
    convert_syslog_to_csv(local_syslog_file, local_csv_output_file)

    # Convert syslog from the remote device to CSV
    remote_syslog_file = './SYSLOG/INTERM DEVICE/syslog.txt'
    remote_csv_output_file = './SYSLOG/INTERM DEVICE/syslog.csv'
    convert_syslog_to_csv(remote_syslog_file, remote_csv_output_file)